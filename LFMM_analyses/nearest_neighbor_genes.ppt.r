...
#import LFMM results
lfmm_results = read.table("data/Nf_LFMM_tables/ppt_lfmm.txt", header = T)
colnames(lfmm_results)
#filter for SNPs with sig relationship to variable
######################
#For ppt use auto adjust p vals
...
p1
pdf("figures/SNP_distance.ppt.pdf")
p1
dev.off()
...
p2
pdf("figures/SNP_per_gene.ppt.pdf")
p2
dev.off()
...
##############################
write.table(gff.dt %>% filter(sig_SNP_count > 0), "data/Nf_LFMM_tables/ppt.gene_SNP_hits.txt", col.names = T, row.names = F, sep = "\t", quote = F)
write.table(lfmm_results.sig, "data/Nf_LFMM_tables/ppt.SNPs.gene_found.nearest_neighbors.txt", col.names = T, row.names = F, sep = "\t", quote = F)
write.table(gff.dt %>% filter(sig_SNP_count > 0) %>% select(geneID), "data/Nf_LFMM_tables/ppt.geneIDs.nearest_neighbors.txt", col.names = F, row.names = F, sep = "\t", quote = F)
