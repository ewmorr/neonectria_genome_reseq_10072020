...
#import LFMM results
lfmm_results = read.table("data/Nf_LFMM_tables/freezeThaw_lfmm.txt", header = T)
colnames(lfmm_results)
#filter for SNPs with sig relationship to variable
#############Can use either manual calibartion or automatic for freeze-thaw, they give the same number of sig. SNPs
...
lfmm_results.sig$distance %>% abs %>% mean #1967
lfmm_results.sig$distance %>% abs %>% median #705
lfmm_results.sig %>% filter(abs(distance) > 0) %>% pull(distance) %>% abs %>% range #19 to 16549
lfmm_results.sig %>% filter(match.type == "in.gene") %>% nrow #13
lfmm_results.sig %>% nrow #65 in gene

gff.dt %>% filter(sig_SNP_count > 0) #32 genes match
gff.dt %>% filter(sig_SNP_count > 1) #13 have >1 SNPs
...
p1 = ggplot(lfmm_results.sig, aes(x = distance)) +
    geom_histogram(breaks = seq(-5000, 5000, 250)) +
...
p1
pdf("figures/SNP_distance.freezeThaw.pdf")
p1
dev.off()
...
p2
pdf("figures/SNP_per_gene.freezeThaw.pdf")
p2
dev.off()
...
##############################
write.table(gff.dt %>% filter(sig_SNP_count > 0), "data/Nf_LFMM_tables/freezeThaw.gene_SNP_hits.txt", col.names = T, row.names = F, sep = "\t", quote = F)
write.table(lfmm_results.sig, "data/Nf_LFMM_tables/freezeThaw.SNPs.gene_found.nearest_neighbors.txt", col.names = T, row.names = F, sep = "\t", quote = F)
write.table(gff.dt %>% filter(sig_SNP_count > 0) %>% select(geneID), "data/Nf_LFMM_tables/freezeThaw.geneIDs.nearest_neighbors.txt", col.names = F, row.names = F, sep = "\t", quote = F)
