...
gi = gl2gi(gl)
gi@ploidy = rep(as.integer(1), 65)

gp = genind2genpop(gi)
#for diveRsity file must be read by function
saveRDS(gp, "Nf_post_SPANDx/LD_filter/Nf.out.filtered.LD_filtered_0.5_10Kb.genpop")


gp.Fst.pairwise = diveRsity::haploDiv(infile = "Nf_post_SPANDx/LD_filter/Nf.out.filtered.LD_filtered_0.5_10Kb.genpop", outfile = "foo.txt", pairwise = T)
